...
def get_scheduler(optimizer, p):
    if p.lr_policy == 'lambda':
        def lambda_rule(epoch):
            lr_l = 1.0 - max(0, epoch + 1 + p.epoch_count - p.niter) / float(p.niter_decay + 1)
            return lr_l
        scheduler = lr_scheduler.LambdaLR(optimizer, lr_lambda=lambda_rule)
    elif p.lr_policy == 'step':
        scheduler = lr_scheduler.StepLR(optimizer, step_size=p.lr_decay_iters, gamma=0.1)
    elif p.lr_policy == 'plateau':
        scheduler = lr_scheduler.ReduceLROnPlateau(optimizer, mode='min', factor=0.2, threshold=0.01, patience=5)
    else:
        return NotImplementedError(f'learning rate policy {p.lr_policy} is not implemented')
    return scheduler
